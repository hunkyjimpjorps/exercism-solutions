...
(define (process-minefield field)
  (for/list ([(posn cell) (in-hash field)])
    (cond
      [(equal? cell #\*) (cons posn #\*)]
      [else (cons posn (count-neighboring-mines posn field))])))

(define (count-neighboring-mines posn field)
  (define sum
...
(define (annotate minefield)
  (let* ([minefield-hash (parse-field minefield)]
         [counts (process-minefield minefield-hash)]
